...
class StockPicking(models.Model):
    ...
    # -------------------------------------------------------------------------
    @api.model
    def force_workflow_ordered_ready(self, order_ids):
        """ Force ordered ready from external:
        """
        return self.with_context(force_order_ids=order_ids).workflow_ordered_ready()
    ...
    def workflow_ordered_ready(self):
        ...
        if force_order_ids:
            domain.append(('order_id', 'in', force_order_ids))
        purchase_lines = line_pool.search(domain)
        ...
        for line in purchase_lines.sorted(
            ...
            # -----------------------------------------------------------------
            # Product of purchase line not received:
            # -----------------------------------------------------------------
